...
import com.automannn.plugin.config.Settings;
...
import com.automannn.plugin.tool.*;
...
public class CodeGenerateServiceImpl implements CodeGenerateService {
    ...
    public void generateByUnifiedConfig(Collection<Template> templates, boolean unifiedConfig, boolean title) {
        ...
        tableInfoList.forEach(tableInfo -> {
            if (StringUtils.isEmpty(tableInfo.getSavePath())) {
                tableInfo.setSaveModuleName(selectedTableInfo.getSaveModuleName());
     ...
     * 生成代码，并自动保存到对应位置，使用统一配置
     *
     * @param frontTemplates     模板
     * @param unifiedConfig 是否使用统一配置
     * @param title         是否显示提示
     */
    @Override
    public void generateFrontByUnifiedConfig(Collection<FrontTemplate> frontTemplates, boolean unifiedConfig, boolean title) {
        // 获取选中表信息
        TableInfo selectedTableInfo = tableInfoService.getTableInfoAndConfig(cacheDataUtils.getSelectDbTable());
        // 获取所有选中的表信息
        List<TableInfo> tableInfoList = tableInfoService.getTableInfoAndConfig(cacheDataUtils.getDbTableList());
        ...
        /*此处，直接适配类型，不再往下进行分化*/
        List<Template> templates= frontTemplates.stream().map(frontTemplate -> new Template(frontTemplate.getName(),frontTemplate.getCode())).collect(Collectors.toList());
    ...
    private void setModulePathAndImportList(Map<String, Object> param, TableInfo tableInfo) {
        ...
        if (!StringUtils.isEmpty(tableInfo.getSaveModuleName())) {
            module = this.moduleManager.findModuleByName(tableInfo.getSaveModuleName());
        }
        if (module != null) {
            // 设置modulePath
